...
function tallenna_line() {
  ...
  muodot.push(["line", x, y, Show.MouseX.value, Show.MouseY.value, Show.Color.value]);
  piirt = 0;
};
...
function tallenna_circle() {
  ...
  if (Show.Mid.checked) {
    ...
    muodot.push(["circle", x, y, r, Show.Color.value, Show.Fill.checked]);
  }
  else {
...
function tallenna_oval() {
  ...
  if (Show.Mid.checked) {
    ...
    muodot.push(["oval", x, y, 1.39*lev, 1.39*kork, Show.Color.value, Show.Fill.checked]);
  }
  else {
    x1 = Show.MouseX.value;
    y1 = Show.MouseY.value;
    ...
    muodot.push(["oval", x_kesk, y_kesk, lev, kork, Show.Color.value, Show.Fill.checked]);
  }
  piirt = 0;
}
...
function tallenna_rect() {
  ...
  if (Show.Mid.checked) {
    lev = 2*lev;
    kork = 2*kork;
    muodot.push(["rect", x-Math.abs(x1-x), y-Math.abs(y1-y), lev, kork, Show.Color.value, Show.Fill.checked]);
  }
  else {
    muodot.push(["rect", Math.min(x, x1), Math.min(y, y1), lev, kork, Show.Color.value, Show.Fill.checked]);
  }
  piirt = 0;
}
...
function piirra_line(x_alku, y_alku, x_loppu, y_loppu, vari) {
...
function piirra_circle(x_keskip, y_keskip, r, vari, fill) {
  vari = typeof vari !== 'undefined' ? vari : "#000000";
  x_keskip -= padd;
  y_keskip -= padd;
  context.beginPath();
...
function piirra_oval(x_keskip, y_keskip, lev, kork, vari, fill) {
  x_keskip -= padd;
  y_keskip -= padd;
...
function piirra_rect(x0, y0, lev, kork, vari, fill) {
  context.beginPath();
...
function tempPiirto() {
  if (piirt == 1) {
    refrsh();
    if (Show.Mid.checked) {
      piirra_line(x-(tempX-x), y-(tempY-y), tempX, tempY, Show.Color.value);
    }
    else {
      piirra_line(x, y, tempX, tempY, Show.Color.value);
    }
  }
  else if (piirt == 2) {
    refrsh();
    context.setLineDash([5]);
    piirra_line(x, y, tempX, tempY, lightgray);
    context.setLineDash([]);
    if (Show.Mid.checked) {
      var r = Math.sqrt(Math.pow(Show.MouseX.value-x, 2)+Math.pow(Show.MouseY.value-y, 2));
      piirra_circle(x, y, r, Show.Color.value, Show.Fill.checked);
    }
    else {
      ...
      piirra_circle(x_keskip, y_keskip, r, Show.Color.value, Show.Fill.checked);
    }
  }
  else if (piirt == 3) {
    refrsh();
    if (Show.Mid.checked) {
      ...
      piirra_oval(x, y, 1.39*lev, 1.39*kork, Show.Color.value, Show.Fill.checked);
    }
    else {
      context.setLineDash([5]);
      ...
      piirra_oval(x_kesk, y_kesk, lev, kork, Show.Color.value, Show.Fill.checked);
    }
  }
  else if (piirt == 4) {
    refrsh();
    if (Show.Mid.checked) {
      piirra_rect(x-Math.abs(tempX-x), y-Math.abs(tempY-y), 2*Math.abs(x-tempX), 2*Math.abs(y-tempY), Show.Color.value, Show.Fill.checked);
    }
    else {
